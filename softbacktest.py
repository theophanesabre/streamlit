...
try:
    if os.path.exists(font_path):
        font_css = f"""
        <style>
            @font-face {{ font-family: 'PASTOROF'; src: url('{font_path}') format('truetype'); }}
            h1[data-testid="stHeading"], .stApp > header h1 {{ font-family: 'PASTOROF', sans-serif !important; font-size: 3rem !important; }}
            ...
            div[data-testid="stNumberInput"] label, div[data-testid="stTextInput"] label, div[data-testid="stRadio"] label, div[data-testid="stCheckbox"] label {{ color: #EEEEEE !important; }}
...
# ==============================================================
# Fonction Principale de Backtesting (INCHANGÉE)
# ==============================================================
def backtest_strategy(csv_filepath_url, initial_equity=5000, ema_short_period=1000,
    ...
    fig = None
    try:
        st.write(f"Chargement des données depuis URL...")
        df = pd.read_csv(csv_filepath_url, encoding='utf-8', on_bad_lines='skip')
    ...
    except FileNotFoundError: st.error(f"Erreur: URL '{csv_filepath_url}' introuvable."); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    except KeyError: st.error(f"Erreur: Colonne 'Date' manquante."); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    except Exception as e: st.error(f"Erreur lecture/traitement CSV depuis URL: {e}"); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    ...
    open_positions=[]; trade_id_counter=0; total_rows=len(df)
    st.write(f"Début boucle backtesting ({total_rows} bougies)...")
    for i, (index, row) in enumerate(df.iterrows()):
        ...
        if current_atr < atr_threshold: continue
...
def plot_single_trade(data_url, trade_info, params):
    """Identique à la version précédente"""
    try:
        entry_time=trade_info['entry_time']; exit_time=trade_info['exit_time']; entry_price=trade_info['entry_price']
        exit_price=trade_info['exit_price']; stop_loss=trade_info['stop_loss']; take_profit=trade_info['take_profit']
        trade_type=trade_info['entry_type']; time_buffer=pd.Timedelta(minutes=60); plot_start_time=entry_time-time_buffer
        ...
        df_full=df_full.set_index('Date').sort_index(); plot_df=df_full[(df_full.index >= plot_start_time) & (df_full.index <= plot_end_time)].copy()
        ...
        plot_df['ema_short']=ta.trend.ema_indicator(plot_df['Close'],window=params['ema_short_period']); plot_df['ema_long']=ta.trend.ema_indicator(plot_df['Close'],window=params['ema_long_period'])
        plot_df['rsi']=ta.momentum.rsi(plot_df['Close'],window=params['rsi_length']); plot_df['atr']=ta.volatility.average_true_range(plot_df['High'],plot_df['Low'],plot_df['Close'],window=params['atr_period'])
        ...
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['ema_short'],mode='lines',name=f'EMA({params["ema_short_period"]})',line=dict(color='lightblue',width=1)),row=1,col=1)
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['ema_long'],mode='lines',name=f'EMA({params["ema_long_period"]})',line=dict(color='yellow',width=1)),row=1,col=1)
        ...
        lemc="rgba(100,100,255,0.5)"; fig.add_vline(x=entry_time,line_width=1,line_dash="dash",line_color=lemc); fig.add_vline(x=exit_time,line_width=1,line_dash="dash",line_color=lemc)
        ...
        if not pd.isna(exit_price): fig.add_trace(go.Scatter(x=[exit_time],y=[exit_price],mode='markers',name='Sortie Pt',marker=dict(symbol='x',color=mec,size=10,line=dict(width=1,color='white'))),row=1,col=1)
        ...
        fig.add_hline(y=params['rsi_overbought'],line_dash="dash",line_color="red",row=2,col=1); fig.add_hline(y=params['rsi_oversold'],line_dash="dash",line_color="lime",row=2,col=1)
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['atr'],mode='lines',name='ATR',line=dict(color='darkgrey',width=1)),row=3,col=1)
        if params['atr_threshold']>0: fig.add_hline(y=params['atr_threshold'],line_dash="dot",line_color="cyan",name='Seuil ATR',row=3,col=1)
        fig.update_layout(title=f"Visualisation Trade #{trade_info.name} ({trade_type.upper()})",xaxis_rangeslider_visible=False,height=800,showlegend=True,legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1), template="plotly_dark")
...
NETLIFY_DATA_URL = "https://lovecrash.online/btc-usd_data_1min.csv" # Votre URL
st.sidebar.caption(f"Données: lovecrash.online/...") # URL raccourcie pour affichage
...
spread_mode_input = st.sidebar.radio("Type de Spread", ["Fixe ($)", "Pourcentage (%)"], index=1, key='spread_mode', help="Simulation spread à l'entrée.")
spread_cost_val = 0.0; spread_pct_val = 0.0; spread_pct_input_raw = 0.0
if spread_mode_input == "Fixe ($)":
    spread_cost_val = st.sidebar.number_input("Spread Fixe ($)", min_value=0.0, value=10.0, step=0.5, format="%.2f", help="Coût fixe.")
    spread_type_arg = "fixed"
else: # Pourcentage (%)
    spread_pct_input_raw = st.sidebar.number_input("Spread (%)", min_value=0.00, max_value=5.0, value=0.02, step=0.005, format="%.3f", help="Ex: 0.02 pour 0.02%.")
    spread_pct_val = spread_pct_input_raw / 100.0; spread_type_arg = "percentage"

st.sidebar.subheader("Indicateurs Techniques")
# --- MODIFICATION: Suppression max_value pour EMA ---
ema_short = st.sidebar.number_input(
    "Période EMA Courte",
    min_value=2, value=21, step=1, format="%d", # max_value retiré
    help="Inférieure à EMA Longue. Min=2."
)
ema_long = st.sidebar.number_input(
    "Période EMA Longue",
    min_value=2, value=55, step=1, format="%d", # max_value retiré
    help="Supérieure à EMA Courte. Min=2."
)
# --- FIN MODIFICATION ---
...
risk_pct_input = st.sidebar.number_input("Risque par Trade (%)", min_value=0.01, max_value=5.0, value=0.5, step=0.01, format="%.2f", help="Pourcentage capital risqué.")
risk_pct = risk_pct_input / 100.0
sl_mode_input = st.sidebar.radio("Type de Stop Loss", ["Pourcentage (%)", "ATR"], index=1, key='sl_mode', help="Méthode calcul SL.")
sl_pct_input = 0.0; atr_multiplier_sl_input = 0.0
if sl_mode_input == "Pourcentage (%)":
    sl_pct_input = st.sidebar.number_input("Stop Loss (%)", min_value=0.01, max_value=10.0, value=0.5, step=0.01, format="%.2f", help="SL en %.")
    sl_type_arg = "percentage"; sl_pct = sl_pct_input / 100.0; atr_multiplier_sl_input = 1.5 # Default ATR mult if % selected
else: # ATR
    atr_multiplier_sl_input = st.sidebar.number_input("Multiplicateur ATR (SL)", min_value=0.1, max_value=10.0, value=1.5, step=0.1, format="%.1f", help="SL = Multiplicateur * ATR.")
    sl_type_arg = "atr"; sl_pct = 0.005 # Default % SL if ATR selected

tp_mult = st.sidebar.number_input("Multiplicateur TP (x Risque)", min_value=0.1, max_value=10.0, value=2.0, step=0.1, format="%.1f", help="TP = Multiplicateur * Distance_SL.")
...
st.sidebar.markdown("---"); st.sidebar.info("Backtester v2.1 (EMA No Limit)")

# --- Zone d'Affichage Principale ---
# ... (Reste de la logique d'affichage identique à la version précédente) ...
...
if st.session_state.results_calculated:
    ...
    if stats:
        ...
        st.subheader("Statistiques Clés"); col1,col2,col3=st.columns(3); col1.metric("Profit ($)",f"{stats.get('Total Profit',0):,.2f}"); col2.metric("Profit (%)",f"{stats.get('Profit (%)',0):.2f}%"); col3.metric("PF",f"{stats.get('Profit Factor',0):.2f}" if stats.get('Profit Factor')!=float('inf') else "Inf")
        ...
        if not trade_history.empty:
            ...
            if st.button("Afficher le Graphique du Trade", key='show_trade_btn'):
                 with st.spinner("Génération graphique trade..."):
                     trade_details = trade_history.iloc[selected_trade_index]; backtest_params = st.session_state.backtest_params
                     single_trade_fig = plot_single_trade(NETLIFY_DATA_URL, trade_details, backtest_params)
