# -*- coding: utf-8 -*-
# app.py (ou le nom que vous préférez)
...
import os # Pour vérifier existence fichier font
import traceback # Pour afficher les erreurs détaillées

# Configuration Matplotlib pour Streamlit
matplotlib.use('Agg')

# --- Début: Configuration Page et Style ---
st.set_page_config(layout="wide", page_title="Lovecrash Backtester")

# Chemin vers le fichier de police (suppose qu'il est dans le même dossier que app.py)
font_path = "PASTOROF.ttf"
font_css = ""
# Vérifie si le fichier existe avant d'essayer de l'inclure
# Note: os.path.exists peut ne pas fonctionner comme prévu sur toutes les plateformes d'hébergement
# Il est plus sûr de tester le chargement et d'avoir une police de repli.
# Pour la simplicité ici, on garde la vérification mais soyez conscient de ses limites.
try:
    if os.path.exists(font_path):
        ...
        <style>
            ...
            /* Cible l'élément H1 généré par st.title */
            /* Ajuster le sélecteur si nécessaire après inspection */
            h1[data-testid="stHeading"], .stApp > header h1 {{
                font-family: 'PASTOROF', sans-serif !important;
                font-size: 3rem !important;
            }}
             /* Styles Globaux pour l'apparence */
            body {{
                font-family: 'Helvetica Neue', sans-serif; /* Police corps */
            }}
            div.main {{ /* Conteneur principal de l'app */
                 background-color: #000000 !important;
                 color: #FFFFFF !important;
             }}
            /* Couleur texte générale */
            div[data-testid="stAppViewContainer"], .stApp {{
                 color: #FFFFFF !important;
            }}
             /* Couleur texte pour les labels des widgets */
    ...
    # Si le fichier n'existe pas, on ne fait rien (pas d'erreur, juste pas de police custom)
except Exception as e:
    # En cas d'erreur imprévue avec os.path.exists ou autre
    st.warning(f"Impossible de vérifier/charger la police personnalisée : {e}")


# Appliquer le titre (sera stylisé par CSS si police chargée)
st.title("Lovecrash")
# --- Fin: Configuration Page et Style ---
...
# ==============================================================
def backtest_strategy(csv_filepath_url, initial_equity=5000, ema_short_period=1000,
                      ...
                      atr_threshold=0.0,
                      spread_type='fixed', spread_cost=0.0, spread_percentage=0.0,
                      take_profit_multiplier=5,
                      progress_placeholder=None,
                      one_trade_at_a_time=True):
    """Effectue le backtest avec toutes les options."""
    fig = None
    # --- Lecture CSV depuis URL ---
    try:
        st.write(f"Chargement des données depuis URL...") # Feedback
        df = pd.read_csv(
            csv_filepath_url,
            # Ajout d'options pour gérer potentiels problèmes
            encoding='utf-8',
            on_bad_lines='skip' # Ignore les lignes mal formées
            )
        st.write("Données brutes chargées. Traitement des dates...")
        # Conversion Date / Index (essentiel)
        df['timestamp'] = pd.to_datetime(df['Date'], errors='coerce')
        df.dropna(subset=['timestamp'], inplace=True) # Supprime lignes où date invalide
        df.set_index('timestamp', inplace=True)
        df = df.sort_index()
        st.write("Index Date/Heure créé.")
    except FileNotFoundError: st.error(f"Erreur: URL '{csv_filepath_url}' introuvable ou inaccessible."); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    except KeyError: st.error(f"Erreur: Colonne 'Date' manquante dans les données chargées."); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    except Exception as e: st.error(f"Erreur lors du chargement/traitement CSV depuis URL: {e}"); return pd.DataFrame(), pd.Series(dtype=float), {}, fig

    # --- Nettoyage et Validation OHLC ---
    ...
    required_length = max(ema_long_period, rsi_length, atr_period) + 1 # +1 pour calcul indicateur
    if len(df) < required_length: st.error(f"Erreur: Pas assez de données valides ({len(df)}) après nettoyage (Min: {required_length})."); return pd.DataFrame(), pd.Series(dtype=float), {}, fig
    st.write("Données OHLC nettoyées.")

    # --- Calcul Indicateurs ---
    try:
        st.write("Calcul des indicateurs...")
    ...
    total_rows=len(df)
    st.write(f"Début de la boucle de backtesting sur {total_rows} bougies...")
    ...
    # --- Finalisation ---
    st.write("Fin de la boucle de backtesting.")
    trade_history_df=pd.DataFrame(closed_trades_history)
    if not trade_history_df.empty:
         st.write("Calcul des statistiques et génération des graphiques...")
         ...
         st.write("Aucun trade exécuté, statistiques par défaut générées.")
...
def plot_single_trade(data_url, trade_info, params):
    """Crée un graphique Plotly pour un seul trade."""
    try:
        ...
        # Charger données depuis URL pour la fenêtre spécifique
        df_full=pd.read_csv(data_url, parse_dates=['Date'], date_parser=lambda x: pd.to_datetime(x, errors='coerce'), encoding='utf-8', on_bad_lines='skip')
        ...
        # Recalculer indicateurs sur la fenêtre
        ...
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['ema_short'],mode='lines',name=f'EMA({params["ema_short_period"]})',line=dict(color='lightblue',width=1)),row=1,col=1) # Couleur plus claire
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['ema_long'],mode='lines',name=f'EMA({params["ema_long_period"]})',line=dict(color='yellow',width=1)),row=1,col=1) # Couleur différente
        lec,ltp,lsl,lex = 'grey','lime','red','fuchsia'; pos_right = "bottom right"; pos_left="top right" # Couleurs vives
        ...
        if not pd.isna(exit_price): # Afficher sortie seulement si elle existe
        ...
        lemc="rgba(100,100,255,0.5)"; # Couleur ligne verticale entrée/sortie
        fig.add_vline(x=entry_time,line_width=1,line_dash="dash",line_color=lemc)
        fig.add_vline(x=exit_time,line_width=1,line_dash="dash",line_color=lemc)
        ms = 'triangle-up' if trade_type=='long' else 'triangle-down'; mc = 'lime' if trade_type=='long' else 'red'; mec='fuchsia' # Couleurs vives
        ...
        if not pd.isna(exit_price):
            fig.add_trace(go.Scatter(x=[exit_time],y=[exit_price],mode='markers',name='Sortie Pt',marker=dict(symbol='x',color=mec,size=10,line=dict(width=1,color='white'))),row=1,col=1)
        # Pane 2: RSI
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['rsi'],mode='lines',name='RSI',line=dict(color='rgb(180,180,255)',width=1)),row=2,col=1) # Couleur RSI
        fig.add_hline(y=params['rsi_overbought'],line_dash="dash",line_color="red",row=2,col=1)
        fig.add_hline(y=params['rsi_oversold'],line_dash="dash",line_color="lime",row=2,col=1)
        # Pane 3: ATR
        fig.add_trace(go.Scatter(x=plot_df.index,y=plot_df['atr'],mode='lines',name='ATR',line=dict(color='darkgrey',width=1)),row=3,col=1)
        if params['atr_threshold']>0: fig.add_hline(y=params['atr_threshold'],line_dash="dot",line_color="cyan",name='Seuil ATR',row=3,col=1) # Couleur seuil ATR
        # Layout
        fig.update_layout(title=f"Visualisation Trade #{trade_info.name} ({trade_type.upper()})",xaxis_rangeslider_visible=False,height=800,showlegend=True,legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1), template="plotly_dark") # Utilise template sombre
        fig.update_yaxes(title_text="Prix",row=1,col=1, gridcolor='rgba(180,180,180,0.3)'); fig.update_yaxes(title_text="RSI",range=[0,100],row=2,col=1, gridcolor='rgba(180,180,180,0.3)'); fig.update_yaxes(title_text="ATR",row=3,col=1, gridcolor='rgba(180,180,180,0.3)')
        fig.update_xaxes(gridcolor='rgba(180,180,180,0.3)') # Couleur grille X
        return fig
    except Exception as e: st.error(f"Erreur génération graphique trade: {e}"); st.error(traceback.format_exc()); return None
...
# --- MODIFIÉ: Utilisation de l'URL directement ---
NETLIFY_DATA_URL = "https://lovecrash.online/btc-usd_data_1min.csv"
st.sidebar.caption(f"Données chargées depuis: {NETLIFY_DATA_URL}") # Affiche l'URL utilisée
# csv_file = st.sidebar.text_input(...) # Input supprimé
# --- FIN MODIFICATION ---
...
spread_mode_input = st.sidebar.radio("Type de Spread", ["Fixe ($)", "Pourcentage (%)"], index=1, key='spread_mode', help="Simulation spread à l'entrée.") # Défaut sur %
spread_cost_val = 0.0; spread_pct_val = 0.0; spread_pct_input_raw = 0.0
if spread_mode_input == "Fixe ($)":
    spread_cost_val = st.sidebar.number_input("Spread Fixe ($)", min_value=0.0, value=10.0, step=0.5, format="%.2f", help="Coût fixe.") # Défaut plus bas
    spread_type_arg = "fixed"; spread_pct_val = 0.0
else: # Pourcentage (%)
    spread_pct_input_raw = st.sidebar.number_input("Spread (%)", min_value=0.00, max_value=5.0, value=0.02, step=0.005, format="%.3f", help="Ex: 0.02 pour 0.02%.") # Défaut plus bas
    spread_pct_val = spread_pct_input_raw / 100.0; spread_type_arg = "percentage"; spread_cost_val = 0.0

st.sidebar.subheader("Indicateurs Techniques")
# ... (Inputs EMA, RSI, ATR - avec valeurs par défaut peut-être plus courantes) ...
ema_short = st.sidebar.number_input("Période EMA Courte", min_value=2, max_value=500, value=21, step=1, format="%d", help="Inférieure à EMA Longue.") # Défaut plus court
ema_long = st.sidebar.number_input("Période EMA Longue", min_value=3, max_value=1000, value=55, step=1, format="%d", help="Supérieure à EMA Courte.") # Défaut plus court
...
risk_pct_input = st.sidebar.number_input("Risque par Trade (%)", min_value=0.01, max_value=5.0, value=0.5, step=0.01, format="%.2f", help="Pourcentage capital actuel risqué.")
risk_pct = risk_pct_input / 100.0
sl_mode_input = st.sidebar.radio("Type de Stop Loss", ["Pourcentage (%)", "ATR"], index=1, key='sl_mode', help="Méthode calcul SL.") # Défaut sur ATR
sl_pct_input = 0.0; atr_multiplier_sl_input = 0.0
if sl_mode_input == "Pourcentage (%)":
    sl_pct_input = st.sidebar.number_input("Stop Loss (%)", min_value=0.01, max_value=10.0, value=0.5, step=0.01, format="%.2f", help="SL en %.") # Défaut plus large
    sl_type_arg = "percentage"; sl_pct = sl_pct_input / 100.0; atr_multiplier_sl_input = 2.0
else: # ATR
    atr_multiplier_sl_input = st.sidebar.number_input("Multiplicateur ATR (SL)", min_value=0.1, max_value=10.0, value=1.5, step=0.1, format="%.1f", help="SL = Multiplicateur * ATR.") # Défaut plus serré
    sl_type_arg = "atr"; sl_pct = 0.005

tp_mult = st.sidebar.number_input("Multiplicateur TP (x Risque)", min_value=0.1, max_value=10.0, value=2.0, step=0.1, format="%.1f", help="TP = Multiplicateur * Distance_SL.") # Défaut plus bas
...
st.sidebar.markdown("---"); st.sidebar.info("Backtester v2.0")
...
if run_button:
    ...
    info_str = f"Lancement (SL:{sl_mode_input}, Spr:{spread_info}, ATR>{atr_filter_threshold:.5f}, Conc:{concurrency_mode})..."
    st.info(info_str)
    progress_placeholder_area = st.empty()
    current_params = {"ema_short_period": ema_short, "ema_long_period": ema_long,"rsi_length": rsi_len, "rsi_oversold": rsi_os, "rsi_overbought": rsi_ob, "atr_period": atr_period_input, "atr_threshold": atr_filter_threshold}

    # Appel backtest_strategy
    th, ec, stats, efig = backtest_strategy(
        csv_filepath_url=NETLIFY_DATA_URL, initial_equity=initial_equity, ema_short_period=ema_short, ema_long_period=ema_long,
        rsi_length=rsi_len, rsi_oversold=rsi_os, rsi_overbought=rsi_ob, risk_percentage=risk_pct, sl_type=sl_type_arg,
        stop_loss_percentage=sl_pct, atr_period=atr_period_input, atr_multiplier_sl=atr_multiplier_sl_input,
        atr_threshold=atr_filter_threshold, spread_type=spread_type_arg, spread_cost=spread_cost_val, spread_percentage=spread_pct_val,
        take_profit_multiplier=tp_mult, progress_placeholder=progress_placeholder_area, one_trade_at_a_time=one_trade_at_a_time_input)

    progress_placeholder_area.empty()
    # Sauvegarde Session State
    st.session_state.trade_history=th; st.session_state.equity_curve=ec; st.session_state.statistics=stats; st.session_state.equity_fig=efig; st.session_state.backtest_params=current_params; st.session_state.results_calculated=True

# Affichage conditionnel des résultats depuis Session State
if st.session_state.results_calculated:
    ...
    if stats:
        ...
        with date_col2: st.markdown(f"**Dernier Trade:**"); st.write(last_date.strftime(date_format) if last_date else "N/A"); st.divider()
        ...
        col7,col8,col9=st.columns(3); col7.metric("Max Loss Str",f"{stats.get('Max Consecutive Losing Trades',0)}"); col8.metric("Avg Loss Str",f"{stats.get('Average Consecutive Losing Trades',0):.1f}"); col9.metric("Final Eq ($)",f"{stats.get('Final Equity',0):,.2f}"); st.divider()
        ...
        if not trade_history.empty:
            ...
            if st.button("Afficher le Graphique du Trade", key='show_trade_btn'):
                 with st.spinner("Génération graphique trade..."):
                     ...
                     single_trade_fig = plot_single_trade(NETLIFY_DATA_URL, trade_details, backtest_params) # Passe l'URL Netlify
