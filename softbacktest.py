# -*- coding: utf-8 -*-
# app.py (Nom recommandé pour Streamlit Cloud)
...
import os # Pour vérifier l'existence du fichier font

matplotlib.use('Agg')

# --- Début: Ajout pour le style ---
...
if os.path.exists(font_path):
    # Injecte le @font-face et applique au h1 si le fichier existe
    font_css = f"""
    <style>
        @font-face {{
        ...
        h1 > div > span {{ /* Sélecteur potentiellement plus spécifique */
            font-family: 'PASTOROF', sans-serif !important;
            font-size: 3rem !important; /* Taille approx. du CSS original */
            /* Autres styles si nécessaire */
        }}
         /* Style global pour correspondre à styles.css */
        body {{
            font-family: 'Helvetica Neue', sans-serif; /* Police corps */
        }}
        /* Cibler le fond de l'application principale */
         div.main {{
             background-color: #000000 !important; /* Fond noir */
             color: #FFFFFF !important; /* Texte blanc */
         }}
         /* Cibler le texte général */
        div[data-testid="stAppViewContainer"] {{
             color: #FFFFFF !important;
        }}
         /* Cibler spécifiquement les widgets si nécessaire */
         div[data-testid="stNumberInput"] label,
         div[data-testid="stTextInput"] label,
         div[data-testid="stRadio"] label {{
              color: #EEEEEE !important; /* Légèrement moins blanc pour les labels */
         }}
         /* Styles pour les métriques (ajuster si besoin) */
         div[data-testid="stMetric"] {{
             background-color: #111111 !important; /* Fond léger pour métriques */
             border-radius: 5px;
             padding: 10px;
             color: #FFFFFF !important;
         }}
         ...
         div[data-testid="stMetric"] > div:nth-of-type(2) {{ /* Valeur de la métrique */
              color: #FFFFFF !important;
         }}
    ...
    st.warning(f"Fichier police '{font_path}' non trouvé. Utilisation de la police par défaut.")

# Appliquer le titre (sera stylisé par le CSS injecté si la police est trouvée)
st.title("Lovecrash")
# --- Fin: Ajout pour le style ---
...
def calculate_statistics(trade_history, equity_curve, initial_equity):
    ...
    # ... (coller le code de calculate_statistics ici) ...
    stats = {}
...
def backtest_strategy(csv_filepath, initial_equity=5000, ema_short_period=1000,
                      # ... (coller le reste de la fonction backtest_strategy ici) ...
    ...
    for i, (index, row) in enumerate(df.iterrows()):
        ...
        if can_enter and (long_condition or short_condition):
            ...
            else: st.error(f"Type spread non reconnu: {spread_type}"); continue
...
def plot_single_trade(csv_filepath, trade_info, params):
    """Identique à la version précédente"""
    # ... (coller le code de plot_single_trade ici) ...
    try:
        ...
        # Utilisation de l'encoding pour potentiels caractères spéciaux et gestion erreurs date
        df_full=pd.read_csv(csv_filepath, parse_dates=['Date'], date_parser=lambda x: pd.to_datetime(x, errors='coerce'), encoding='utf-8', on_bad_lines='skip')
...
# (coller ici la partie sidebar de la réponse précédente, incluant le choix de spread) ...
...
spread_mode_input = st.sidebar.radio("Type de Spread", ["Fixe ($)", "Pourcentage (%)"], key='spread_mode', help="Comment simuler le spread à l'entrée.")
spread_cost_val = 0.0; spread_pct_val = 0.0; spread_pct_input_raw = 0.0
if spread_mode_input == "Fixe ($)":
    spread_cost_val = st.sidebar.number_input("Spread Fixe ($)", min_value=0.0, value=60.0, step=0.5, format="%.2f", help="Coût fixe ajouté/soustrait du prix d'entrée.")
    spread_type_arg = "fixed"
else: # Pourcentage (%)
    spread_pct_input_raw = st.sidebar.number_input("Spread (%)", min_value=0.00, max_value=5.0, value=0.05, step=0.01, format="%.3f", help="Ex: 0.05 pour 0.05%.")
...
st.sidebar.markdown("---"); st.sidebar.info("Backtester v2.0 (Final)")


# --- Zone d'Affichage Principale ---
# (coller ici la partie principale de la réponse précédente,
# incluant la logique session state et l'affichage des résultats / visualisation) ...
...
if run_button:
    ...
    th, ec, stats, efig = backtest_strategy(
        csv_filepath=csv_file, initial_equity=initial_equity, ema_short_period=ema_short, ema_long_period=ema_long,
    ...
    st.session_state.trade_history = th; st.session_state.equity_curve = ec; st.session_state.statistics = stats; st.session_state.equity_fig = efig
